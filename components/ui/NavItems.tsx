...
const NavItems = () => {
    ...
    return (
        <div className={`${styles.navItemsCont} h-full`}>
            {
                navItems.map((item) => (
                    <a href={item.href} key={item.title} onClick={() => handleHover(!drop)} className={`flex cursor-pointer relative gap-4 h-full hover:text-primary justify-between items-center group transition-all duration-300 ease-in-out ${drop && item.dropdown ? "text-primary" : ""}`}
                        ...
                        {item.dropdown && <div>
                            ...
                            <div className='absolute  w-5 md:w-20 h-16 lg:h-14  lg:w-35 z-2 inset-0' onMouseEnter={() => handleHover(item.dropdown)} onMouseLeave={() => handleHover(false)}></div>
                        </div>
