...
const NavItems = () => {
    ...
    return (
        <div className={`${styles.navItemsCont} h-full`}>
            {
                navItems.map((item) => (
                    <a href={item.href} key={item.title} onClick={() => handleHover(!drop)} className={`flex cursor-pointer relative gap-4 h-full hover:text-primary justify-between items-center group transition-all duration-300 ease-in-out ${drop && item.dropdown ? "text-primary" : ""}`} onMouseEnter={() => handleHover(item.dropdown)} onMouseLeave={() => handleHover(false)}>
                        {item.title}
                        {item.dropdown && <div>
